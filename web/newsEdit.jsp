...
<form method="post" action="" id="ajaxForm">
  <div class="release_news">
    <div class="news_title">
      ...
      <input type="text" placeholder="填写标题(应控制在32个汉字以内）" id="title" name="title" value="${news.title}" maxlength="32">
    </div>
    <div class="common_text">
      <label class="text_center">主图</label>
      <input type="text" id="mainImg" name="main_img" size="20" style=" width: 460px;" readonly value="${news.main_img}"
    ...
    <div class="column_name">
      ...
      <select class="column_name_release" id="prod_id" name="prod_id">
        <c:forEach var="prod" items="${prodList}">
          <option value="${prod.id}"  <c:if test="${news.prod_id eq prod.id}">selected="selected"</c:if> >${prod.title}</option>
        </c:forEach>
      </select>
    </div>
    <div class="column_name">
      ...
      <select class="column_name_release" id="chnlCode" name="type">
        <c:forEach var="newsChnl" items="${productTypeList}">
          <option value="${newsChnl}" <c:if test="${news.type eq newsChnl}">selected="selected"</c:if> >${newsChnl}</option>
        </c:forEach>
      </select>
    </div>
    <div class="column_name">
      ...
      <select class="column_name_release" name="is_top">
        <option value="0" <c:if test="${news.is_top == 0}">selected="selected"</c:if> >否</option>
        <option value="1" <c:if test="${news.is_top == 1}">selected="selected"</c:if> >是</option>
      </select>
    </div>
    <div class="column_content">
                        <textarea id="container" rows="100" cols="100" placeholder="" name="content"
                                  style="height:500px;" class="release_news_content">${news.content}</textarea>
  ...
  $(function () {
    $(".release_news_ok_btn").click(function () {
      ...
      $('#ajaxForm').ajaxSubmit({
        type: "post",  //提交方式
        dataType: "json", //数据类型
        url: _CTX + "/compusnews/news/update.json",//默认是form action
        success: function (data) {
          alert("修改成功");
